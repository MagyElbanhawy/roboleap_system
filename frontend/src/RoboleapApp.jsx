    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ marginBottom:32 }}>
          ...
          <img src={roboleapLogo} alt="Roboleap Academy logo" style={S.loginBrandLogo} />
        ...
        <div style={{ marginTop:20, padding:"14px 16px", background:COLORS.surface2, borderRadius:10, fontSize:11.5, color:COLORS.textMid }}>
          ...
          {[["admin","admin123"],["finance","finance123"],["secretary","secretary123"],["instructor1","instr123"],["magy","123"],["tagrid","123"],["mawada","123"]].map(([u,p])=>(
...
function DashboardPage({ user }) {
  const api = useApi();
  const canViewFinance = (user.is_superuser || user.is_staff || user.role === "admin" || user.role === "finance") && user.username !== "mawada";
  const canAddTransaction = canViewFinance || user.username === "mawada";
  const showInstructorWorkload = user.username !== "mawada";
  ...
  const openCourse = async (course) => {
    ...
    try {
      const [sessions, enrollments] = await Promise.all([
        api.get(`/api/courses/${course.id}/sessions/`),
        api.get(`/api/courses/${course.id}/students/`),
      ]);
      setCourseSessions(sessions?.results || sessions || []);
      setCourseEnrollments(enrollments?.results || enrollments || []);
    } catch (e) {
      toast(e.message || "Unable to load course details");
    } finally {
  ...
  const openSession = async (session) => {
    ...
    try {
      const attendance = await api.get(`/api/attendance/?session=${session.id}`);
      setSessionAttendance(attendance?.results || attendance || []);
    } catch (e) {
      toast(e.message || "Unable to load attendance");
    } finally {
  ...
  return (
    <div>
      ...
      <div style={{ display:"grid", gridTemplateColumns: showInstructorWorkload ? "1.2fr 1fr" : "1fr", gap:16 }}>
        <div style={S.card}>
            ...
            <table style={S.table}>
              ...
              <tbody>
                {courses.slice(0,6).map(c => (
                  <tr key={c.id} style={{ cursor:"pointer" }} onClick={() => openCourse(c)}>
                    <td style={S.td}>
                      <div style={{ fontWeight:500, color:COLORS.accent }}>{c.name}</div>
      ...
      {selectedCourse && (
        <div style={{ marginTop:20 }}>
          <CourseDetailView
            ...
            onBack={closeCourse}
            onSelectSession={session => session ? openSession(session) : setSelectedSession(null)}
          />
        </div>
      )}
...
function AddTransactionModal({ api, currentUser, onClose, onSaved }) {
  const isLimitedMawada = currentUser?.username === "mawada";
  const [form, setForm] = useState({
    transaction_type: "income",
    category: isLimitedMawada ? "student_payment_summary" : "sponsorship",
    custom_category: "",
    title: "",
  ...
  useEffect(() => {
    if (!isLimitedMawada || !form.related_student) {
      setEnrollments([]);
      setForm(p => ({ ...p, enrollment_id: "" }));
      return;
    }

    setLoadingEnrollments(true);
    api.get(`/api/students/${form.related_student}/enrollments/`)
      .then(d => setEnrollments(d?.results || d || []))
      .catch(() => setEnrollments([]))
      .finally(() => setLoadingEnrollments(false));
  }, [form.related_student, isLimitedMawada]);

  const categories = isLimitedMawada ? [
  ...
  const save = async () => {
    ...
    if (isLimitedMawada && !form.related_student) {
      setErr("Please select the student who paid the course.");
      return;
    }
    if (isLimitedMawada && !form.enrollment_id) {
      setErr("Please select the course or competition the student is paying for.");
      return;
    }
    ...
    try {
      ...
      if (form.related_student) payload.related_student = Number(form.related_student);
      if (form.enrollment_id) payload.enrollment_id = Number(form.enrollment_id);
      if (selectedEnrollment) {
        payload.event_name = form.event_name || selectedEnrollment.course?.name;
      } else if (form.event_name) {
        payload.event_name = form.event_name;
      }
      if (form.event_location) payload.event_location = form.event_location;
      ...
      const message = e.data?.detail || (e.data && JSON.stringify(e.data)) || e.message || "Failed to save transaction";
      setErr(message);
    } finally {
      setSaving(false);
    }
  ...
  return (
    <Modal title="Add Transaction" onClose={onClose}>
      ...
      {isLimitedMawada && form.related_student && (
        <Field label="Course / Competition">
          <select style={{ ...S.select, width:"100%" }} value={form.enrollment_id}
            onChange={e => setForm(p => ({ ...p, enrollment_id: e.target.value }))}
            disabled={loadingEnrollments}>
            <option value="">{loadingEnrollments ? "Loading courses…" : "— Select course or competition —"}</option>
            {enrollments.map(enrollment => (
              <option key={enrollment.id} value={enrollment.id}>
                {enrollment.course?.name || `Enrollment #${enrollment.id}`} {enrollment.balance != null ? `(${fmt.egp(enrollment.balance)} due)` : ""}
              </option>
            ))}
          </select>
        </Field>
              ...
              <table style={S.table}>
                ...
                <tbody>
                  {courses.length === 0 ? (
                    ...
                    <tr key={c.id} style={{ cursor:"pointer" }} onClick={() => openCourse(c)}>
                      <td style={S.td}><span style={{ fontWeight:600, color:COLORS.accent }}>{c.name}</span></td>
...
function CourseDetailView({ course, sessions, enrollments, attendance, selectedSession, loadingCourseDetail, loadingAttendance, onBack, onSelectSession }) {
  const enrollmentById = Object.fromEntries(enrollments.map(e => [e.id, e]));

  return (
    <div>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20, gap:12 }}>
        <button style={S.btn("ghost")} onClick={onBack}>← Back to courses</button>
        <div style={{ flex:1 }}>
          <h1 style={{ fontSize:20, fontWeight:700, letterSpacing:-0.5, margin:0 }}>{course.name}</h1>
          <div style={{ fontSize:12, color:COLORS.textMid, marginTop:6 }}>
            {course.batch ? `${course.batch} · ` : ""}{fmt.egp(course.fee)} · {course.status}
          </div>
        </div>
      </div>

      {loadingCourseDetail ? <Spinner /> : (
        <div style={{ display:"grid", gridTemplateColumns:"1.25fr 0.95fr", gap:16 }}>
          <div style={S.card}>
            <div style={{ fontSize:14, fontWeight:600, color:COLORS.text, marginBottom:14 }}>Sessions</div>
            {sessions.length === 0 ? <EmptyState icon="◆" message="No sessions found for this course." /> : (
              <table style={S.table}>
                <thead><tr>
                  <th style={S.th}>Title</th>
                  <th style={S.th}>Date</th>
                ...
                <tbody>
                  {sessions.map(s => (
                    <tr key={s.id} style={{ cursor:"pointer" }} onClick={() => onSelectSession(s)}>
                      <td style={S.td}><strong style={{ color:COLORS.accent }}>{s.title}</strong></td>
                      <td style={S.td}>{fmt.date(s.session_date)}</td>
                      <td style={S.td}>{s.attendance_count ?? 0}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div style={S.card}>
            <div style={{ fontSize:14, fontWeight:600, color:COLORS.text, marginBottom:14 }}>Enrolled Students</div>
            {enrollments.length === 0 ? <EmptyState icon="◇" message="No students enrolled yet." /> : (
              <table style={S.table}>
                <thead><tr>
                  <th style={S.th}>Student</th>
                ...
                <tbody>
                  {enrollments.map(e => (
                    <tr key={e.id}>
                      ...
                      <td style={S.td}>{fmt.egp(e.total_paid)} / {fmt.egp(e.total_due)}</td>
                      <td style={S.td}><span style={S.statusBadge(e.payment_status === "paid" ? "active" : e.payment_status === "partial" ? "upcoming" : "ended")}>{e.payment_status}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {selectedSession && (
        <div style={{ ...S.card, marginTop:16 }}>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:14 }}>
            <div>
              <div style={{ fontSize:14, fontWeight:600, color:COLORS.text }}>Attendance for {selectedSession.title}</div>
              <div style={{ fontSize:12, color:COLORS.textMid }}>{fmt.date(selectedSession.session_date)}</div>
            </div>
            <button style={S.btn("ghost")} onClick={() => onSelectSession(null)}>Clear session</button>
          </div>
          {loadingAttendance ? <Spinner /> : (
            attendance.length === 0 ? (
              <EmptyState icon="⚠" message="No attendance recorded for this session." />
            ) : (
              <table style={S.table}>
                <thead><tr>
                  <th style={S.th}>Student</th>
                ...
                <tbody>
                  {attendance.map(att => {
                    const enrollment = enrollmentById[att.enrollment] || {};
                    return (
                      <tr key={att.id}>
                        <td style={S.td}>{enrollment.student_name || att.student_name}</td>
                        <td style={S.td}><span style={S.statusBadge(att.status)}>{att.status}</span></td>
                        <td style={S.td}>{enrollment.payment_status || "unknown"}</td>
                        <td style={S.td}>{enrollment.balance != null ? fmt.egp(enrollment.balance) : "—"}</td>
                      </tr>
                    );
                  })}
                </tbody>
      ...
      <nav style={S.sidebar}>
        <div style={S.sidebarLogo}>
          <div style={S.logoMark}>
            ...
            <img src={roboleapLogo} alt="Roboleap Academy logo" style={S.brandLogo} />
          </div>
        </div>
